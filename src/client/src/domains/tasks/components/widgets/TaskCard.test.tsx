...
describe('TaskCard', () => {
  it('renders task title', () => {
    const task = createTaskItem({ title: 'Buy groceries' });
    render(<TaskCard task={task} />);
    expect(screen.getByText('Buy groceries')).toBeInTheDocument();
  });

  it('renders priority badge when priority is not None', () => {
    const task = createTaskItem({ priority: Priority.High });
    render(<TaskCard task={task} />);
    expect(screen.getByText('High')).toBeInTheDocument();
  });

  it('renders tags', () => {
    const task = createTaskItem({ tags: ['bug', 'urgent'] });
    render(<TaskCard task={task} />);
  ...
  it('renders due date', () => {
    ...
    const task = createTaskItem({ dueDate: today.toISOString() });
    render(<TaskCard task={task} />);
    expect(screen.getByText('Today')).toBeInTheDocument();
  });

  it('applies line-through style for completed tasks', () => {
    const task = createTaskItem({ status: TaskStatus.Done });
    render(<TaskCard task={task} />);
  ...
  it('calls onComplete when check button is clicked', async () => {
    ...
    const task = createTaskItem({ id: 'task-1' });
  ...
  it('calls onSelect when card is clicked', async () => {
    ...
    const task = createTaskItem({ id: 'task-2' });
