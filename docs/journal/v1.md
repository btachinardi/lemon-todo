# Development Journal — v1

> **Source**: Extracted from docs/JOURNAL.md
> **Status**: Active
...
The first document ([Product Vision](../product/vision.md)) captured everything known at that stage:
...
Every technology in the stack was researched ([operations/research.md](../operations/research.md)), verifying:
...
This is where the planning leveled up. Instead of jumping to domain design, detailed storyboards were written from the USER's perspective:
...
After scenarios, the official requirements document was created, incorporating everything learned. Key changes:
...
With requirements solid, the domain was designed:
...
Before touching code, the rules of engagement were established:
...
The old L1/L2/L3 labels tried to do both jobs at once and created confusion. The new model is cleaner: Architecture Tiers flow data top-down (from URL to pixels), while the Component Taxonomy flows bottom-up (small primitives compose into bigger domain-aware pieces).
...
See [roadmap/capability-tiers.md](../roadmap/capability-tiers.md) for future capability tiers.
